    ...
    <div class="mx-auto max-w-5xl px-4 py-10">
            ...
            <div>
                ...
                <div class="aspect-[4/3] overflow-hidden rounded-2xl bg-stone-100">
                    @if ($photos->isNotEmpty())
                        <img src="{{ $photos->first()->url() }}" alt="{{ $photos->first()->alt }}"
            ...
            <div>
                <div class="flex items-center gap-3">
                    ...
                    <span @class([
                        ...
                        'bg-stone-200 text-stone-600' => in_array($pet->status, ['adopted', 'unavailable']),
                    ])>{{ $pet->status }}</span>
                </div>

                <p class="mt-1 text-stone-600">
                    {{ $pet->breed?->name ?? $pet->species->name }} · {{ $pet->species->name }}
                </p>

                <dl class="mt-6 grid grid-cols-2 gap-4 text-sm">
                    <div><dt class="text-stone-500">Age</dt><dd class="font-medium">{{ $pet->ageForHumans() ?? 'Unknown' }}</dd></div>
                    <div><dt class="text-stone-500">Sex</dt><dd class="font-medium capitalize">{{ $pet->sex }}</dd></div>
                    <div><dt class="text-stone-500">Size</dt><dd class="font-medium capitalize">{{ $pet->size ?? '—' }}</dd></div>
                    <div><dt class="text-stone-500">Color</dt><dd class="font-medium">{{ $pet->color ?? '—' }}</dd></div>
                    <div><dt class="text-stone-500">Location</dt><dd class="font-medium">{{ $pet->location ?? '—' }}</dd></div>
                ...
                <div class="mt-4 flex flex-wrap gap-2">
                    @foreach (['vaccinated' => 'Vaccinated', 'sterilized' => 'Sterilized', 'good_with_kids' => 'Good with kids', 'good_with_pets' => 'Good with pets'] as $flag => $label)
                        @if ($pet->$flag)
                            <span class="rounded-full bg-emerald-50 px-3 py-1 text-xs text-emerald-700">✓ {{ $label }}</span>
                        @endif
                    @endforeach
                </div>

                @if ($pet->description)
                    <p class="mt-6 leading-relaxed text-stone-700">{{ $pet->description }}</p>
                @endif
            </div>
        </div>
        ...
        @if ($pet->status === 'available')
            <div id="apply" class="mt-12 rounded-2xl border border-stone-200 bg-white p-6 sm:p-8">
                ...
                <p class="mt-1 text-sm text-stone-600">Fill in your details and our team will reach out to arrange a meet.</p>
                ...
                <form method="POST" action="{{ route('pets.apply', $pet) }}" class="mt-6 grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        <button class="rounded-full bg-amber-600 px-6 py-3 font-medium text-white shadow-sm transition hover:bg-amber-700">
                            Submit application
                        </button>
                    </div>
                </form>
